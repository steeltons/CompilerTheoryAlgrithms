...
def __repair_bad_rules(P_bad, lambda_rules, normal_rules):
	...
	for pair in P_bad:
		...
		for symbol in rule_symbols & lambda_rules & normal_rules: 
			...
			for rule in filter(lambda rule: symbol in rule, new_rules): # Убираем лябда терминал из всех существующих правил
				tt = rule.replace(symbol,'')
				if tt != '':
					new_rules = new_rules | {tt}

		new_rules = new_rules | {pair[key]} # Добавляем вывод
